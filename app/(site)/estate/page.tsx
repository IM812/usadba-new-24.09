...
export const metadata: Metadata = {
  title: "Усадьба",
  description: "Бревенчатый дом 250 м² с гостиной и камином, четырьмя спальнями и оборудованной кухней. Дом сдаётся целиком.",
}

const houseNotes = [
  { title: "Кухня", text: "Полностью оборудованная кухня, посудомоечная машина и всё необходимое для приготовления еды во время отдыха." },
...
export default function EstatePage() {
  ...
  return (
    <>
      ...
      <Section tone="base">
        <Container size="wide">
          <div data-reveal><SectionHeading eyebrow="Дом" title="Реальные комнаты, без придуманных названий" lead="Показываем дом таким, какой он есть. Если важна конкретная схема кроватей, подтвердим её для состава вашей компании перед бронированием." /></div>
          ...
          <article data-reveal className="mt-24 lg:mt-32">
            ...
            <div className="mt-10 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4">{bedrooms.images?.map((src, index) => <figure key={src} className="relative aspect-3/4 overflow-hidden rounded-2xl bg-secondary"><Image src={src} alt={`Одна из четырёх спален усадьбы, фотография ${index + 1}`} fill sizes="(max-width: 640px) 50vw, 25vw" className="object-cover" /></figure>)}</div>
          </article>
        </Container>
      </Section>

      <Section tone="deep"><Container size="wide"><div data-reveal className="grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:gap-20"><SectionHeading eyebrow="Детали" title="То, о чём обычно спрашивают" /><div className="grid gap-x-12 gap-y-10 sm:grid-cols-2">{houseNotes.map((note) => <div key={note.title} className="flex flex-col gap-3 border-t border-border pt-5"><h3 className="font-display text-xl font-semibold text-accent">{note.title}</h3><p className="text-pretty text-[15px] leading-relaxed text-muted-foreground">{note.text}</p></div>)}</div></div></Container></Section>
      ...
      <BookingCta title="Посмотрите дом своими глазами" lead="Расскажите, сколько вас и на какие даты — подтвердим размещение и пришлём дополнительные фотографии комнат." image="/images/estate/house-facade.jpg" imageAlt="Фасад бревенчатого дома усадьбы среди сосен" />
    </>
  )
}
